...
const IGNORED_CHARACTERS = [".", "_"];

// filetypes to force unix line ending if enabled
const FORCE_UNIX_FILE_TYPES = [".txt", ".text", ".js", ".json", ".jsn", ".xml", ".md", ".htm", ".html", ".css", ".csv", ".php", ".cfg", ".ini", ".list", ".lst"];
...
function getDefs(file) {
  if (file.slice(-4).includes(".js")) {
    ...
    data = data.replace(/\/\/.*/g, ""); // ignore: // ...
    data = data.replace(/[A-Z\.]+\.majorPatchVersion/gim, "majorPatchVersion");
          ...
          if (manifest.defs[packet[0]]) {
            ...
            if (Array.isArray(manifest.defs[packet[0]])) {
              if (!manifest.defs[packet[0]].includes(packetVer)) {
                ...
                manifest.defs[packet[0]].sort((a, b) => {
                  return a - b;
                });
              }
            }
...
function jsonify(obj) {
  ...
  let lists = obj.match(/\[[^]+?\].*/gim);
  if (lists)
    for (let list of lists) {
      obj = obj.substring(0, obj.indexOf(list)) + list.replace(/[ \n\t]*/gim, "") + obj.substring(obj.indexOf(list) + list.length);
    }
  return obj;
}
